...
module.exports = async (bot, msg) => {
    try {
        ...
        let cmdFile =
            ...
            bot.commands.find(
                (cmdFile) =>
                    cmdFile.aliases && cmdFile.aliases.includes(command)
            );
        ...
        if (!msg.channel.nsfw && cmdFile.nsfw) return embeds.nsfw(msg);

        let isOwner = bot.config.owners.includes(msg.author.id);
        if (cmdFile.ownerOnly && !isOwner) return;
        if (
            cmdFile.permissions &&
            !msg.member.permissions.has(cmdFile.permissions) &&
            !isOwner
        )
        ...
        if (
            ...
            !msg.guild.me.permissions.has(cmdFile.botPermissions)
        )
